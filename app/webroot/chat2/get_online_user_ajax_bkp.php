...
if(mysql_num_rows($appointDet)>0){
	...
	if($conv_end_time !=""){
		$_SESSION['conv_end_time']=$conv_end_time;
	}
}

if(isset($_SESSION['conv_end_time']) && $_SESSION['conv_end_time']!=""){
	//echo $cur_time=date('H:i');
	//echo $_SESSION['conv_end_time'];
	if($cur_time == $_SESSION['conv_end_time']){
...
if(isset($_SESSION['appoint_id']) && $_SESSION['appoint_id']!=""){
	...
	if(mysql_num_rows($chkAppoint)>0){
		...
		echo "<script>alert('Now you are Timed Out');window.top.location.href='".SITEURL."/dashboard';</script>";
